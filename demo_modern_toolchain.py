...
def main():
    ...
    print(f"✅ Dataset created: {type(sales_data)} with {getattr(sales_data, 'shape', 'unknown shape')}")
    ...
    print(f"  • Missing Data: {quality_report['dataset_overview']['missing_percentage']:.1f}%")
    ...
    print(f"  • Pipeline Status: {'✅ Success' if pipeline_result['success'] else '❌ Failed'}")
    ...
    tracker = ExperimentTracker(
        project_name="modern-toolchain-demo",
    ...
    processed_data['high_profit'] = (processed_data['profit_margin'] > processed_data['profit_margin'].median()).astype(int)
    ...
    print(f"  • Visualizations: {'✅' if evaluation_report.get('yellowbrick_plots') else '📊 Basic'}")
    ...
    if optimized_params.get('max_depth') is not None and optimized_params['max_depth'] <= 0:
    ...
    print(f"✅ Processed dataset: {pipeline_result['final_shape'][0]:,} rows, {pipeline_result['final_shape'][1]} columns")
    print(f"✅ Data quality score: {quality_report['data_quality_score']:.1f}/100")
    print(f"✅ ML experiments tracked: {experiment_summary['total_runs']}")
    print(f"✅ Model explanations: {explanation_results['method_used']}")
    print(f"✅ Hyperparameter optimization: {optimization_results['method']} with {optimization_results['n_trials']} trials")
    ...
    print("\n📚 Next Steps:")
    print("• Try the Level 7 challenge: challenges/level_7/challenge_1_modern_toolchain.md")
