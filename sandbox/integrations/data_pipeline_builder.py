...
class DataPipelineBuilder:
    ...
    def create_data_validation_suite(self, 
                                   data: pd.DataFrame, 
                                   suite_name: str = "default_suite") -> Dict[str, Any]:
        """
    ...
    def _create_ge_suite(self, data, suite_name):
        ...
        try:
            ...
            row_count = len(data)
            expectations.append(f"expect_table_row_count_to_be_between({int(row_count * 0.9)}, {int(row_count * 1.1)})")
            ge_df.expect_table_row_count_to_be_between(int(row_count * 0.9), int(row_count * 1.1))
    ...
    def _create_basic_validation(self, data, suite_name):
        ...
        for col, missing_count in missing_data.items():
            if missing_count > 0:
                ...
                expectations.append(f"Column '{col}': {missing_count} missing values ({missing_pct:.1f}%)")
                if missing_pct > 50:
                    issues.append(f"High missing data in column '{col}': {missing_pct:.1f}%")
        
        # Check for duplicate rows
        duplicate_count = data.duplicated().sum()
        if duplicate_count > 0:
            duplicate_pct = (duplicate_count / total_rows) * 100
            expectations.append(f"Found {duplicate_count} duplicate rows ({duplicate_pct:.1f}%)")
            if duplicate_pct > 10:
    ...
    def create_data_pipeline(self, 
                           ...
                           transformations: List[Callable] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            for i, transform_func in enumerate(transformations):
                try:
                    transform_name = getattr(transform_func, '__name__', f'transformation_{i}')
    ...
    def create_data_quality_report(self, data: pd.DataFrame) -> Dict[str, Any]:
        ...
        for col in data.columns:
            col_analysis = {
                ...
                "uniqueness_ratio": data[col].nunique() / total_rows if total_rows > 0 else 0
            }
            ...
            if col_analysis["missing_percentage"] > 0:
                col_quality -= min(col_analysis["missing_percentage"], 50)  # Max 50 point deduction
        ...
        report["data_quality_score"] = sum(quality_scores) / len(quality_scores) if quality_scores else 0
    ...
    def _generate_quality_recommendations(self, report: Dict[str, Any]) -> List[str]:
        ...
        for col, analysis in report["column_analysis"].items():
            if analysis["missing_percentage"] > 20:
                recommendations.append(f"📊 Column '{col}': High missing rate ({analysis['missing_percentage']:.1f}%)")
            
            if analysis["uniqueness_ratio"] == 1 and len(report["column_analysis"]) > 10:
                recommendations.append(f"🔑 Column '{col}': Appears to be an identifier - consider removing for modeling")
            
            if analysis["uniqueness_ratio"] == 0:
                recommendations.append(f"📈 Column '{col}': No variation detected - consider removing")
    ...
    def get_tool_comparison(self) -> Dict[str, Any]:
        ...
        comparison = {
            "great_expectations": {
                "status": "✅ Available" if self.great_expectations_available else "❌ Not installed",
            ...
            "pandera": {
                "status": "✅ Available" if self.pandera_available else "❌ Not installed",
            ...
            "prefect": {
                "status": "✅ Available" if self.prefect_available else "❌ Not installed",
