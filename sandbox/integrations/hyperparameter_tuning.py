...
class HyperparameterOptimizer:
    ...
    def optimize_model(self,
        ...
        if method == "optuna" and self.optuna_available:
            results = self._optimize_with_optuna(objective_function, param_space, n_trials, study_name)
        elif method == "hyperopt" and self.hyperopt_available:
            results = self._optimize_with_hyperopt(objective_function, param_space, n_trials, study_name)
        elif method == "skopt" and self.skopt_available:
            results = self._optimize_with_skopt(objective_function, param_space, n_trials, study_name)
        else:
            results = self._optimize_with_grid_search(objective_function, param_space, study_name)
    ...
    def _optimize_with_optuna(self, objective_function, param_space, n_trials, study_name):
        ...
        try:
            ...
            return {
                ...
                "trials_dataframe": study.trials_dataframe().to_dict('records') if hasattr(study, 'trials_dataframe') else []
            }
            
        except Exception as e:
            print(f"Optuna optimization failed: {e}")
            return self._optimize_with_grid_search(objective_function, param_space, study_name)
    
    def _optimize_with_hyperopt(self, objective_function, param_space, n_trials, study_name):
        ...
        try:
            ...
            for param_name, param_config in param_space.items():
                if param_config["type"] == "float":
                    ...
                    hp_space[param_name] = hp.choice(param_name, param_config["choices"])
    ...
    def _optimize_with_skopt(self, objective_function, param_space, n_trials, study_name):
        ...
        try:
            ...
            for param_name, param_config in param_space.items():
                if param_config["type"] == "float":
                    ...
                    grid_space[param_name] = list(range(
                        param_config["low"],
                        min(param_config["high"] + 1, param_config["low"] + 5)  # Max 5 points
                    ))
                elif param_config["type"] == "categorical":
...
'''
    ...
    def get_tool_comparison(self) -> Dict[str, Any]:
        ...
        comparison = {
            "optuna": {
                "status": "✅ Available" if self.optuna_available else "❌ Not installed",
            ...
            "hyperopt": {
                "status": "✅ Available" if self.hyperopt_available else "❌ Not installed",
            ...
            "scikit_optimize": {
                "status": "✅ Available" if self.skopt_available else "❌ Not installed",
    ...
    def export_optimization_history(self, output_file: str = "optimization_history.json"):
        """
