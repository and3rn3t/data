...
class ModelExplainer:
    ...
    def explain_prediction(self, 
        ...
        if method == "shap" and self.shap_available:
            results.update(self._explain_with_shap(model, X_train_np, X_explain_np, feature_names))
        
        # LIME explanations
        elif method == "lime" and self.lime_available:
            results.update(self._explain_with_lime(model, X_train_np, X_explain_np, feature_names))
        
        # Simple feature importance fallback
        else:
            results.update(self._explain_simple(model, X_train_np, X_explain_np, feature_names))
        ...
        try:
            ...
            return {
                ...
                "explanations": {
                    ...
                    "interpretation": self._interpret_shap_values(shap_values, feature_names)
                },
                "plot_created": True
            }
    ...
    def _explain_simple(self, model, X_train, X_explain, feature_names):
        ...
        try:
            ...
            return {
                ...
                "error": str(e),
                "feature_importance": {},
                "explanations": {"summary": "Explanation failed - model may not be compatible"}
            }
    ...
    def _interpret_lime_explanation(self, explanation):
        ...
        interpretation = []
        if top_positive:
            interpretation.append(f"Features supporting prediction: {', '.join(top_positive)}")
        if top_negative:
            interpretation.append(f"Features opposing prediction: {', '.join(top_negative)}")
    ...
    def create_model_evaluation_report(self, 
        ...
        if task_type == "classification":
            results.update(self._create_classification_report(model, X_test, y_test, y_pred))
        else:
            results.update(self._create_regression_report(model, X_test, y_test, y_pred))
    ...
    def get_tool_comparison(self) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "yellowbrick": {
                "status": "✅ Available" if self.yellowbrick_available else "❌ Not installed",
