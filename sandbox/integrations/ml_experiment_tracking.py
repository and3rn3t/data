...
class ExperimentTracker:
    ...
    def __init__(
        self,
        project_name: str = "data-science-sandbox",
    ...
    def _init_trackers(self):
        ...
        if self.mlflow_available:
            try:
                ...
                print(
                    f"✅ MLflow initialized - Tracking URI: {self.mlflow.get_tracking_uri()}"
                )
    ...
    def log_artifact(self, file_path: str, artifact_path: Optional[str] = None):
        ...
        if self.fallback_logs:
            self.fallback_logs[-1]["artifacts"].append(
                {"file_path": file_path, "artifact_path": artifact_path, "type": "file"}
            )
    ...
    def create_comparison_demo(self) -> Dict[str, Any]:
        ...
        comparison = {
            "mlflow": {
                "status": (
                    "✅ Available" if self.mlflow_available else "❌ Not installed"
                ),
            ...
            "wandb": {
                "status": (
                    "✅ Available" if self.wandb_available else "❌ Not installed"
                ),
