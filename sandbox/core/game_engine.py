...
from config import LEVELS, BADGES, CATEGORIES, BASE_DIR
...
class GameEngine:
    ...
    def load_progress(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "level_progress": {str(i): {"unlocked": i == 1, "completed": False, "score": 0} 
                             for i in range(1, 7)}
        }
    ...
    def check_level_completion(self):
        ...
        completed_challenges = [c for c in self.progress["challenges_completed"] 
        ...
        if len(completed_challenges) >= len(level_challenges) * 0.8:  # 80% completion
            ...
            if next_level > current_level:
                print(f"🎊 Level {current_level} Complete! Level {next_level} Unlocked!")
    ...
    def get_level_challenges(self, level: int) -> List[str]:
        ...
        if os.path.exists(challenges_dir):
            return [f for f in os.listdir(challenges_dir) if f.endswith('.py') or f.endswith('.ipynb')]
        return []
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_levels": 6,
            "completion_rate": len(self.progress["challenges_completed"]) / max(1, self.count_total_challenges()) * 100
        }
        ...
        for level in range(1, min(current_level + 1, 7)):
            if self.progress["level_progress"][str(level)]["unlocked"]:
                ...
                for challenge in challenges:
                    completed = f"level_{level}_{challenge}" in self.progress["challenges_completed"]
    ...
    def launch_jupyter(self):
        ...
        try:
            ...
            print("❌ Jupyter Lab not available. Please install: pip install jupyterlab")
            print("🔄 Alternative: Open notebooks manually in your preferred environment")