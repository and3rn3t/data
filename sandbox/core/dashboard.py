...
class Dashboard:
    ...
    def create_sidebar(self):
        ...
        # Navigation Pills
        st.sidebar.markdown("<h4 style='margin: 20px 0 16px 0; color: var(--text-secondary);'>🧭 Navigation</h4>", unsafe_allow_html=True)
        ...
        st.sidebar.markdown("<h4 style='margin: 20px 0 16px 0; color: var(--text-secondary);'>🚀 Quick Actions</h4>", unsafe_allow_html=True)
    ...
    def show_dashboard(self):
        ...
        st.markdown("<h2 style='margin: 30px 0 20px 0;'>📊 Learning Dashboard</h2>", unsafe_allow_html=True)
            ...
            for level in range(1, 7):
                ...
                progress_data.append({
                    ...
                    'Status': 'Completed' if level_status['completed'] else 
                             'Active' if level_status['unlocked'] else 'Locked',
                    'Progress': 100 if level_status['completed'] else 
                              65 if level_status['unlocked'] else 0
                })
            ...
            if current_level <= 6:
                ...
                if challenges:
                    ...
                    for i, challenge in enumerate(challenges[:3]):  # Show first 3
                        ...
                        status_color = "var(--ios-green)" if completed else "var(--ios-blue)"
        ...
        with col1:
            # Recent badges
            recent_badges = self.game.progress["badges_earned"][-3:] if self.game.progress["badges_earned"] else []
            if recent_badges:
                st.markdown("<h4 style='color: var(--text-secondary);'>🏆 Latest Badges</h4>", unsafe_allow_html=True)
        ...
        with col2:
            ...
            st.markdown("<h4 style='color: var(--text-secondary);'>🔥 Learning Stats</h4>", unsafe_allow_html=True)
        ...
        with col3:
            # Quick actions
            st.markdown("<h4 style='color: var(--text-secondary);'>⚡ Quick Actions</h4>", unsafe_allow_html=True)
        ...
        for level_num, level_info in LEVELS.items():
                ...
                if status["unlocked"]:
                    ...
                    if challenges:
                        ...
                        for i, challenge in enumerate(challenges[:5]):
                            completed = f"level_{level_num}_{challenge}" in self.game.progress["challenges_completed"]
                            dot_color = "var(--ios-green)" if completed else "rgba(255,255,255,0.3)"
                            st.markdown(f"""
            ...
            with col3:
                if status["unlocked"] and not status["completed"]:
                    if st.button(f"▶️", key=f"start_{level_num}", help=f"Start Level {level_num}"):
                        st.session_state.page = 'Challenges'
    ...
    def show_challenges(self):
        ...
        if level_status["unlocked"]:
            ...
            if challenges:
                ...
                completed_challenges = [c for c in self.game.progress["challenges_completed"] 
                ...
                for i, challenge in enumerate(challenges, 1):
                    ...
                    with col3:
                        ...
                        if st.button(button_text, key=f"challenge_{challenge_id}", disabled=button_disabled):
                            if completed:
    ...
    def show_badges(self):
        ...
        completion_rate = (len(earned_badges) / total_badges) * 100 if total_badges > 0 else 0
    ...
    def show_progress(self):
        ...
        with col1:
            ...
            exp_data = pd.DataFrame({
                'Date': dates,
                'Experience': [i * 25 + (i**1.2) * 10 + (i * 5 * (i % 3)) for i in range(30)],
            ...
            fig.update_layout(
                plot_bgcolor='rgba(0,0,0,0)',
                paper_bgcolor='rgba(0,0,0,0)',
                font=dict(family="SF Pro Display, -apple-system, sans-serif", color='#1C1C1E'),
                showlegend=False,
        ...
        with col1:
            ...
            skills_data = pd.DataFrame({
                'Skill': ['Data Cleaning', 'Visualization', 'Statistics', 'Machine Learning', 'Python', 'Analysis'],
                'Proficiency': [85, 70, 60, 45, 90, 75],
                'Category': ['Technical', 'Technical', 'Analytical', 'Advanced', 'Programming', 'Analytical']
            })
            
            # Create horizontal bar chart with iOS colors
            fig = px.bar(skills_data, x='Proficiency', y='Skill', 
                        color='Category',
                        orientation='h',
                        color_discrete_map={
                            'Technical': '#007AFF',
            ...
            fig.update_layout(
                ...
                xaxis=dict(range=[0, 100], showgrid=True, gridcolor='rgba(142, 142, 147, 0.2)'),
        ...
        with col2:
            ...
            time_data = pd.DataFrame({
                'Category': ['Practice Challenges', 'Theory & Reading', 'Projects', 'Review & Analysis'],
                'Hours': [25, 15, 20, 8],
            ...
            fig.update_layout(
                ...
                showlegend=False,
                margin=dict(t=50, b=50, l=50, r=50),
                annotations=[dict(text=f'{sum(time_data["Hours"])}h<br>Total', x=0.5, y=0.5, 
                                font_size=20, showarrow=False, font=dict(color='#1C1C1E', weight='bold'))]
            )
        ...
        fig = px.imshow(heatmap_data, 
                       color_continuous_scale=['#F2F2F7', '#007AFF'],
                       aspect='auto')
    ...
    def show_settings(self):
        ...
        with col2:
            ...
            analytics = st.checkbox("Share anonymous usage data", value=True, 
        ...
        with col2:
            ...
            if st.button("🔄 Reset All Data", key="reset_all", type="secondary"):
                ...
                if st.button("⚠️ I'm Sure, Reset Everything", key="confirm_reset_all", type="primary"):
                    self.game.reset_progress()
